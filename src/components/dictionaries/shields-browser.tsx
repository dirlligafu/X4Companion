...
import type { ShieldCatalogItem } from "@/types/save";
import {
  buildFlatLayout,
  buildSeriesLayout,
...
type Props = { shields: ShieldCatalogItem[] };
...
function ShieldDetail({ shield: s }: { shield: ShieldCatalogItem }) {
  const sid = `shield-${s.macro_id.replace(/[^a-zA-Z0-9_-]/g, "_")}`;
  return (
    <div className={cn(
      ...
      {s.description && (
        <p className="mb-5 border-b border-border/70 pb-4 text-sm leading-relaxed text-muted-foreground whitespace-normal">{s.description}</p>
      )}

      {(() => {
        const imgUrl = (iconCatalogue as Record<string, string>)[s.macro_id];
        return (
          <div className={cn("grid grid-cols-1 gap-8", imgUrl ? "md:grid-cols-3 md:gap-0" : "md:grid-cols-2 md:gap-0")}>
            <DetailSection id={`${sid}-recharge`} title="Recharge" className="md:border-r md:border-border/60 md:pr-6">
              {s.recharge?.max != null && <DetailRow label="Capacity">{s.recharge.max.toLocaleString()} HP</DetailRow>}
              {s.recharge?.rate != null && <DetailRow label="Rate">{fmt(s.recharge.rate, 1)} HP/s</DetailRow>}
              {s.recharge?.delay != null && <DetailRow label="Delay">{s.recharge.delay.toFixed(1)} s</DetailRow>}
              {s.hull != null && <DetailRow label="Hull HP">{s.hull.toLocaleString()}</DetailRow>}
            </DetailSection>

            <DetailSection id={`${sid}-price`} title="Economy" className={imgUrl ? "md:border-r md:border-border/60 md:px-6" : "md:pl-6"}>
              {s.price && (
                <>
                  <DetailRow label="Avg price">{s.price.average.toLocaleString()}</DetailRow>
                  <DetailRow label="Min / Max">{s.price.min.toLocaleString()} / {s.price.max.toLocaleString()}</DetailRow>
                </>
              )}
            </DetailSection>
            ...
            {imgUrl && (
              <div className="flex items-center justify-center md:pl-6">
                <img src={imgUrl} alt={s.macro_id} className="max-h-32 max-w-full object-contain opacity-90" />
              </div>
            )}
          </div>
        );
      ...
      <footer className="mt-5 flex flex-col gap-2 border-t border-border/70 pt-4 text-[11px] text-muted-foreground sm:flex-row sm:flex-wrap sm:items-baseline sm:gap-x-8 sm:gap-y-1">
        {s.basename && (
          <p>
            ...
            <span className="text-foreground/90">{s.basename}</span>
          </p>
        )}
        {s.owners.length > 0 && (
          <p><span className="font-medium text-foreground">Owners</span><span className="mx-1.5 text-border">·</span>{s.owners.map(factionLabel).join(", ")}</p>
        )}
        <p className="break-all"><span className="font-medium text-foreground">Macro</span><span className="mx-1.5 text-border">·</span><span className="font-mono text-[0.7rem] text-foreground/90">{s.macro_id}</span></p>
      </footer>
    </div>
  );
}
...
const COL_COUNT = 8; // chevron + name + faction + mk + capacity + rate + delay + avg price

export function ShieldsBrowser({ shields }: Props) {
  ...
  function shieldDataRow(s: ShieldCatalogItem) {
    const isExpanded = expandedId === s.macro_id;
    return (
      <Fragment key={s.macro_id}>
        <TableRow className="cursor-pointer" onClick={() => toggleExpand(s.macro_id)}>
          <TableCell className="text-muted-foreground pr-0">
            {isExpanded ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
          </TableCell>
          <TableCell className="max-w-56">
            <EquipmentCatalogNameCell name={s.name} playerUsable={s.player_usable} />
          </TableCell>
          <TableCell className="text-center text-xs">{factionLabel(s.faction)}</TableCell>
          <TableCell className="text-center font-mono text-xs">{s.mk ?? "—"}</TableCell>
          <TableCell className="text-right font-mono text-xs">{fmt(s.recharge?.max)}</TableCell>
          <TableCell className="text-right font-mono text-xs">{fmt(s.recharge?.rate, 1)}</TableCell>
          <TableCell className="text-right font-mono text-xs">{s.recharge?.delay != null ? `${s.recharge.delay.toFixed(1)} s` : "—"}</TableCell>
          <TableCell className="text-right font-mono text-xs">{s.price ? s.price.average.toLocaleString() : "—"}</TableCell>
        </TableRow>
        {isExpanded && (
  ...
  const sizes = useMemo(() => {
    const set = new Set(shields.map(s => s.size).filter((s): s is string => s !== null));
          ...
          <Table stickyRoot className="min-w-full">
            <TableHeader className="sticky top-0 z-20 bg-card shadow-sm [&_tr]:border-b">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="w-24 text-right">Regen/s</TableHead>
                <TableHead className="w-16 text-right">Delay</TableHead>
                    ...
                    {layoutMode === "series"
                      ? (seriesLayout.sizeToFamilies[size] ?? []).map(({ family, items }) => (
                          <Fragment key={`${size}-${family}`}>
                            ...
                            {items.map(s => shieldDataRow(s))}
                          </Fragment>
                        ))
                      : (flatItems ?? []).map(s => shieldDataRow(s))}
                  </Fragment>
                );
              })}
            </TableBody>
